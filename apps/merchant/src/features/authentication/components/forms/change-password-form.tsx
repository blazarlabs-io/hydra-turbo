      ...
      .then((userCredential) => {
        ...
        updatePassword(updatedUser as User, values.newPassword)
          .then(() => {
            ...
            toast({
              variant: "default",
              title: "toasts.userSettings.passwordChanged.title",
              description: "toasts.userSettings.passwordChanged.description",
            });
          })
          .catch((error) => {
            ...
            toast({
              variant: "destructive",
              title: "toasts.globals.error.title",
              description: "toasts.globals.error.description",
            });
          });
      })
      .catch((error) => {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="w-full space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                {"manageAccount.changePassword.dialog.currentPassword.label"}
              </FormLabel>
              <FormControl>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                {"manageAccount.changePassword.dialog.newPassword.label"}
              </FormLabel>
              <FormControl>
                <div className="relative flex items-center rounded-md border border-border bg-background text-foreground">
                  <Input
        ...
        <div className="flex w-full items-center justify-end gap-4">
          <DialogClose className="px-4 py-2 text-sm">
            {"manageAccount.changePassword.dialog.buttons.cancelButtonLabel"}
          </DialogClose>
